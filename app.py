...
with aba_dashboard:
    ...
    if not df_clinico.empty:
        ...
        st.markdown("---")
        st.subheader("Correlação entre Atividade Física e Consumo de Alimentos Calóricos por Nível de Obesidade")
        fig_box = px.box(
        ...
        st.markdown("---")
        st.subheader("Matriz de Correlação das Variáveis Clínicas e Comportamentais")
        st.markdown("Entenda quais variáveis numéricas possuem forte associação positiva (valores próximos a +1, em azul) ou inversa (próximos a -1, em vermelho).")
        
        # Definindo as colunas numéricas exatamente como na sua análise exploratória (EDA)
        ...
        if cols_presentes:
            # Calcula a matriz de correlação do Pandas
            corr_matrix = df_clinico[cols_presentes].corr()
            
            # Cria o Heatmap interativo idêntico ao da imagem do seu Seaborn
            fig_heatmap = px.imshow(
                corr_matrix,
                text_auto='.2f',  # Exibe o valor numérico com 2 casas decimais dentro dos quadrados
                aspect="auto",
                color_continuous_scale=px.colors.diverging.RdBu,  # Vermelho para negativo, Azul para positivo
                zmin=-1, zmax=1
            )
            ...
            st.info("**Insight Clínico:** O mapa destaca como variáveis biométricas (como Peso) se correlacionam de forma direta ou inversa com os hábitos diários e a frequência de alimentação da base hospitalar.")
